...
### discount based on HW share
result$peer <- result$peer * 0.55
...
### discount for HW share
result_p$peer <- result_p$peer * 0.55
...
f1a <- daa %>%
  ...
  geom_segment(
    # data = data.frame(),
    aes(
      x = 5 - 0.45,      # Matches the exact left boundary of the "Extra high subsidy" bar
      xend = 5 + 0.45,   # Matches the exact right boundary of the "Extra high subsidy" bar
      y = 55,          # Your original yintercept value
      yend = 55
    ),
    color = "darkblue",
    linetype = "dashed",
  ...
  annotate(
    "text",
    x = 2.5,             # Placed right after the bar bounds (5 + 0.45 = 5.45)
    y = 55,            # Aligned with the line's y-value
    label = "Homeowner 55%",
    color = "darkblue",
    hjust = 0,           # Left-aligned text so it pushes rightward away from the bar
...
# 
# 
# hlines1 <- tibble::tribble(
#   ~tech, ~y_intercept, ~line_label,
#   "Electric vehicles", 84, "Base: 0.84"
# )

hlines <- tibble::tribble(
  ~tech, ~y_intercept, ~line_label,
  "Electric vehicles", 66, "Base: 0.66" # based on 1.45 EV/ adoters
)
...
# hlines <- tibble::tribble(
#   ~tech, ~y_intercept, ~line_label,
#   "Electric vehicles", 63, "Base: 0.42"
# ) %>% 
#   mutate(tech = factor(tech, levels = c("PV + Storage","Electric vehicles","Heat pumps")))
...
d1_dac <- data %>%
  ...
  mutate(
    across(
      -c(tech, dac),
      ~ ifelse(dac == 0, .x * 0.62, .x * 0.43) # homeowners 
    )
  ) %>% 
  ...
  left_join(
    data %>% # only homeowners 
      ...
      mutate(
        across(rent,
          ~ ifelse(dac == 0, .x * 0.38, .x * 0.57) # renters
        )
      ), by = c("tech","dac")
  ) %>% 
...
result_dac <- result_dac %>%
  mutate(peer = if_else(dac == "Non_DAC",
                            peer * 0.62,
                            peer * 0.43))
...
daa1 <- d %>% 
  mutate(across(c(value,start,end), ~ .x*0.925)) # adjusting new building stocks

add_values <- tibble(
  tech = rep(c("PV + Storage","Heat pumps","Induction stoves"), each = 3),
  id    = rep(c(2, 3, 4), 3),
  value = rep(0.5, 9) * 7.5 * d_w[2] # new stock
) %>% 
  mutate(dac = "DAC") %>% 
  rbind(
    tibble(
...
line_labels <- data.frame(
  dac = factor(c("Non-DAC", "DAC"), levels = c("Non-DAC", "DAC")),
  y_val = c(62, 43),
  label_text = c("Homeowner 62%", "Homeowner 43%")
)
...
f2a <- daa %>%
  ...
  geom_segment(
    data = line_labels,
    aes(
      x = 5 - 0.45,     # Starts at the left edge of the 5th bar (Extra high subsidy)
      xend = 5 + 0.45,  # Ends at the right edge of the 5th bar
      y = y_val,        # 62 for Non-DAC, 43 for DAC
      yend = y_val
    ),
    color = "darkblue",
    linetype = "dashed",
    linewidth = 1,
    inherit.aes = FALSE # Prevents inheriting global x-axis stage mappings
  ) +
  
  geom_text(
    data = line_labels,
    aes(
      x = 1.5,          # Placed just to the right of the 5th bar
      y = y_val,        # 62 for Non-DAC, 43 for DAC
      label = label_text
    ),
    color = "darkblue",
    hjust = 0,          # Left-aligned so text pushes outward away from the bar
    size = 3.5, 
    fontface = "italic",
    inherit.aes = FALSE
  ) +
...
hlines <- tibble::tribble(
  ~tech, ~y_intercept, ~line_label,
  "Electric vehicles", 66, "Base: 0.66"
) 
...
d1_dac <- data %>%
  ...
  mutate(
    across(
      -c(tech, dac),
      ~ ifelse(dac == 0, .x * 0.55, .x * 0.55) # homeowners 
    )
  ) %>% 
  ...
  left_join(
    data %>% # only homeowners 
      ...
      mutate(
        across(rent,
               ~ ifelse(dac == 0, .x * 0.45, .x * 0.45) # renters
        )
      ), by = c("tech","dac")
  ) %>% 
...
f2a <- daa %>%
  ...
  filter(ST %in% c("Renters","Homeowners","Subsidy")) %>% 
  ...
  annotate(
    "text",
    x = 1.5,             # Placed right after the bar bounds (5 + 0.45 = 5.45)
    y = 55,            # Aligned with the line's y-value
    label = "Homeowner 55%",
    color = "darkblue",
    hjust = 0,           # Left-aligned text so it pushes rightward away from the bar
...
p_out <- ggarrange(f2a2, f2b2, ncol = 2, widths = c(2, 1),
            common.legend = T, legend = "right")
...
rates <- list(c(-0.07), # lower peer effects for HP
              c(-0.06), # lower for IC
              c(-0.14,-0.23))
