...
for(i in seq(from = 100, to = 15000, by = 100)){
  pt <- data %>% 
    mutate(future = ifelse(storage_plans %in% c("Yes", "Maybe") & solar_pv_plans %in% c("Yes", "Maybe") &
                             solstor_wtp_dv < i | PS == 1, 1, 0)) %>% 
    group_by(dac) %>% 
...
pess <- data.frame(subsidy = c(7200, 7500, 3000, 350),
...
f4a <- curv %>% 
  ...
  ggplot()+
  geom_smooth(aes(x = subsidy, y = mean, group = dac, color = dac)) +
...
p <- curv %>% 
  ...
  mutate(dac = recode(dac,
                      ...
                      '1' = "DAC"),
         dac = factor(dac, levels = c("DAC","Non-DAC"))) %>% 
  ggplot(aes(x = subsidy, y = mean, group = dac, color = dac)) +
...
f4b <-final_smooth_data %>% 
  ...
  mutate(dac = recode(dac,
         ...
         dac = factor(dac, levels = c("DAC","Non-DAC"))) %>% 
  ggplot()+
  geom_smooth(aes(x = subsidy, y = change*1000, group = dac, color = dac)) +
  geom_vline(data = pess, aes(xintercept = subsidy), linetype = "dashed", color = "gray70") +
  geom_vline(data = opt, aes(xintercept = subsidy), linetype = "dotted", color = "gray20") +
